...
The `seeker` package also includes an example shell script for fetching and processing a microarray dataset. Download the file to your working directory:

```{r, eval = FALSE}
urlBase = 'https://raw.githubusercontent.com/hugheylab/seeker/master/inst/extdata/'
for (filename in c('run_seeker_array.R', 'run_seeker_array.sh', 'GSE25585.yml')) {
  download.file(paste0(urlBase, filename), filename)}
```
...
Alternatively, you could modify line 8 of run_seeker_array.sh to use GSE25585.yml instead of providing the arguments directly:
```{sh, eval = FALSE}
#!/bin/sh
...
The output will appear in your working directory. You can follow `seekerArray()`'s progress using the log file. To process a different dataset, modify the shell script accordingly.
