...
To process microarray data, use the `seekerArray()` function. Currently `seekerArray()` can process data from [NCBI GEO](https://www.ncbi.nlm.nih.gov/geo/) and [ArrayExpress](https://www.ebi.ac.uk/arrayexpress/). It can also process raw Affymetrix data stored locally. The main inputs are arguments for `study`, `geneIdType`, and `platform`. You can also supply list of parameters, similar to but much simpler than for `seeker()`. A template yaml file for the parameters is shown below.
...
To run `seekerArray()`, you can either provide the values directly or in a named list, as shown below.
...
study = 'GSE25585'
geneIdType = 'ensembl'
seekerArray(study, geneIdType, parentDir = '.')

# Using params
params = list(study = study, geneIdType = geneIdType)
seekerArray(params = params, parentDir = '.')
```
