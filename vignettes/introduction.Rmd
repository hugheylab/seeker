...
To process microarray data, use the `seekerArray()` function. Currently `seekerArray()` is targeted at processing data from [NCBI GEO](https://www.ncbi.nlm.nih.gov/geo/) and [ArrayExpress](https://www.ebi.ac.uk/arrayexpress/). The main input is again a list of parameters, but much simpler than for `seeker()`. A template yaml file for the parameters is shown below.
