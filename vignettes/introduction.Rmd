...
To process microarray data, use the `seekerArray()` function. Currently `seekerArray()` can process data from [NCBI GEO](https://www.ncbi.nlm.nih.gov/geo/) and [ArrayExpress](https://www.ebi.ac.uk/arrayexpress/). It can also raw Affymetrix data stored locally. The main input is again a list of parameters, but much simpler than for `seeker()`. A template yaml file for the parameters is shown below.
