...
readr::write_csv(metadata, file.path(study, 'metadata_ena.csv'))
...
`getMetadata` with `host == 'sra'` is not very useful, since the most important metadata can only be downloaded manually using the SRA Run Selector (https://www.ncbi.nlm.nih.gov/sra/docs/sradownload/). Nice move, SRA.

```{r, eval = FALSE}
metadataSra = getMetadata(study, host = 'sra')
